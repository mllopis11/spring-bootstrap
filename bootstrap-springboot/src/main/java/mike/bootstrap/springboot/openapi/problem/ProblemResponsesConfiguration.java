...
class ProblemResponsesConfiguration {
	...
	public Map.Entry<String, ApiResponse> http401() {
		return this.buildSimpleResponse(HTTP_401, "Invalid authentication",
					new ProblemExample("no token", "Authentication token not provided", http401NoTokenProblem),
					new ProblemExample("invalid token", "Authentication token is invalid", http401InvalidTokenProblem) );
	}
	...
	private Map.Entry<String, ApiResponse> buildSimpleResponse(String code, String description, Problem problem) {
		...
		ApiResponse response = new ApiResponse().description(description)
				.content(new Content().addMediaType(APPLICATION_PROBLEM_JSON_VALUE,
							new MediaType().example(problem).schema(new Schema<Problem>().$ref("#/components/schemas/Problem"))));
		
		return new AbstractMap.SimpleEntry<>(code, response);
	}
	
	private Map.Entry<String, ApiResponse> buildSimpleResponse(String code, String description, ProblemExample... problems) {
		
		Map<String, Example> examples = Stream.of(problems)
				.map(pbl -> new Example()
									.summary(pbl.getSummary())
									.description(pbl.getDescription()).value(pbl.getExample()))
				.collect(Collectors.toMap(Example::getSummary, example -> example));
		
		ApiResponse response = new ApiResponse().description(description)
				.content(new Content().addMediaType(APPLICATION_PROBLEM_JSON_VALUE,
							new MediaType().example(examples).schema(new Schema<Problem>().$ref("#/components/schemas/Problem"))));
		
		return new AbstractMap.SimpleEntry<>(code, response);
	}
	
	private static class ProblemExample {
		
		private final String summary;
		private final String description;
		private final Problem example;
		
		ProblemExample(String summary, String description, Problem example) {
