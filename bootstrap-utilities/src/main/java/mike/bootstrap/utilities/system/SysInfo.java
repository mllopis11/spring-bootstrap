...
/**
 * Provides general informations about the system (pid, hostname ...) and shortcuts for the JVM
 * properties.
 * 
 * @author Mike (2021-02)
 * @since 1.8
...
public class SysInfo {
    ...
    /**
     * Set main class informations. If the method is invoked multiple time, any action is done.
     * 
        ...
        if (mainClazz == null) {
            ...
            if (mainClazz == null) {
                throw new ClassCastException(
                        "SysInfo::setMainClass: no such 'main' method for class: "
    ...
    public static boolean isUnix() {
        return OS_NAME.toLowerCase().startsWith("linux")
                || OS_NAME.toLowerCase().startsWith("sunos")
    ...
    public static String platform() {
        return String.format("Platform: os=%s, arch=%s (%d cores), jre-verion=%s, username=%s",
                OS_NAME, System.getProperty("os.arch"), SysInfo.cores(),
                System.getProperty("java.specification.version"), USER_NAME);
    }
     ...
     * @throws ApplicationUncheckedException if the port is invalid or the address is not available
     */
    public static void hostAddressAlreadyBound(String hostname, String port) {
        ...
        if (myHost.isBlank() || Utils.isPortNotValid(myPort)) {
            throw new ApplicationErrorException(
                    "bindHostAddress: invalid hostname or port argument");
        }

        try (var socket = new ServerSocket(myPort, 0, InetAddress.getByName(hostname));) {
            // Nothing to do here: Address is free
        } catch (IOException ioe) {
            throw new ApplicationErrorException("bindHostAddress: address=%s:%s, causedBy: %s",
                    hostname, port, ioe.getMessage());
        }
    }
}
