 ...
 * <p>
 * Application informations are read from "META-INF/build-info.properties".
 * If you are using MAVEN, the properties <i>application.name</i> and <i>organization.name</i>
 * should be set in your POM. 
 * 
 * @author Mike (2021-02)
 * @since 15
 */
public class AppInfo {
    ...
    static {
        ...
        Stream.of(buildInfoFile, "META-INF/build-info.properties")
        	.map(Resource::new)
        	.filter(Resource::found)
        	.findFirst()
        	.ifPresentOrElse( res -> { 
        		try {
    ...
    /**
     * Returns the runtime directory specified by the property 'container.run-directory'.
     * If the property is not set, returns the default directory './run/${node}'.
     *  
    ...
    public static String banner() {
        return String.format("%s - %s %s (copyrights: %s) - released: %s", 
        		name(), module(), version(), company, release);
    }
    ...
    public static String starter() {
        return String.format("%s (node: %s) started with pid %s on %s at %s", 
        		module(), node(), SysInfo.pid(), SysInfo.hostname(), Dates.zNow());
    }
    ...
    public static String footer() {
        ...
        return String.format("%s (node: %s) terminated at %s (uptime: %s)", 
        		module(), node(), Dates.zNow(), uptime);
    }
