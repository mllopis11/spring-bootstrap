    ...
    public static class RegexExpression {
        ...
        RegexExpression(Pattern pattern, String value, String expression) {
            ...
            this.regexName = Optional.of(pattern.matcher(Strings.strip(value)))
                    .filter(Matcher::matches).map(m -> m.replaceAll(expression))
                    .orElseThrow(() -> new IllegalArgumentException(
                            String.format("value does not match pattern: %s (pattern: %s)", value,
                                    pattern.pattern())));
        ...
        public String get() {
            ...
            if (this.finalName.contains("%")) {
                throw new IllegalStateException(
                        String.format("remaining unresolved key(s): %s", this.finalName));
            }
