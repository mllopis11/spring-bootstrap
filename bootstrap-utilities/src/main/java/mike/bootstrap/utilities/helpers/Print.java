...
public class Print {
	...
	/**
	 * Set timestamp format. If an invalid format is provided, the
	 * default format is kept.
	 * 
	 * @param format timestamp format
	 */
	public static void timestamp(String format) { 
		try { dtf =  DateTimeFormatter.ofPattern(format); } 
		catch (IllegalArgumentException e) { /* Ignore: keep default format */ }
	}
