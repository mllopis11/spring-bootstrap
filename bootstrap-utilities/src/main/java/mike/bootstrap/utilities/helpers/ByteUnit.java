...
/**
 * ByteUnit represents bit size at given unit granularity and provides utility methods to manipulate
...
public enum ByteUnit {
    ...
    private static final Pattern sizePattern = Pattern
            .compile("(\\d{1,18})((b|k|kb|m|mb|g|gb|t|tb))");
    ...
    /**
     * Compute bytes corresponding to the given size pattern. An IllegalArgumentException is thrown
     * if the argument is invalid or if the value exceeds the max. value for the given unit. The
     * unit max. value is {@link java.lang.Long#MAX_VALUE} divide by the unit bytes.
     * 
    ...
    public static long from(String sizeWithUnit) {
        ...
        return UNITS.stream()
                ...
                .mapToLong(v -> sValue * v.base).findAny()
                .orElseThrow(() -> new IllegalArgumentException(
                        "size exceeds the max. size for the unit"));
    }
    ...
    public static String format(long bytes) {
        ...
        return UNITS.stream()
                ...
                .map(v -> new DecimalFormat("#.##" + v.unit).format(v.size(bytes))).findAny()
                .orElse("0b");
    }
}
