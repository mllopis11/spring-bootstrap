...
public enum ByteUnit {
    ...
    /**
     * Compute bytes corresponding to the given size pattern. An
     * IllegalArgumentException is thrown if the argument is invalid or if the value
     * exceeds the max. value for the given unit. The unit max. value is
     * {@link java.lang.Long#MAX_VALUE} divide by the unit bytes.
     * 
     * @param sizeWithUnit a string representation of the expected size (ex. 2k)
     * @return computed bytes according to the granularity unit (ex.: 2k returns
     *         2048)
     */
    public static long from(String sizeWithUnit) {
	String sizeUnit = Strings.strip(sizeWithUnit).toLowerCase();
	...
	return UNITS.stream().filter(v -> sUnit.equals(v.unit) && sValue <= v.maxSize).mapToLong(v -> sValue * v.base)
		.findAny().orElseThrow(() -> new IllegalArgumentException("size exceeds the max. size for the unit"));
    }

    /**
     * @param bytes size in bytes
     * @return the best representation for the given size (ex.: 6.9kb for 7069
     *         bytes)
     */
    public static String format(long bytes) {

	return UNITS.stream().filter(v -> bytes >= v.base)
		.map(v -> new DecimalFormat("#.##" + v.unit).format(v.size(bytes))).findAny().orElse("0b");
    }
}
