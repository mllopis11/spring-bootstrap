...
public class SSLCertificateConfiguration {
    ...
    /**
     * Configure SSL from the filename read from the JVM system property 'ssl.properties'
     * if exists otherwise from the default certificates filename 'certificates.properties'.
     * 
    ...
    public static void setTrustStore(String trustStore) {
        ...
        if ( ! store.isEmpty() ) {
        	...
        	if ( Files.isReadable(absoluteStorePath) ) {
        		...
        		throw new ApplicationErrorException("no such readable SSL certificate truststore: %s", absoluteStorePath);
        	}
        }
    }
    ...
    /**
     * @return true if the trustStore JVM system property (javax.net.ssl.trustStore) is set otherwise false
     */
