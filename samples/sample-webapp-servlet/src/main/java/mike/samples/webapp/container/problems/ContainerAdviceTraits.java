...
public interface ContainerAdviceTraits extends AdviceTrait {
	...
	default ResponseEntity<Problem> handleResourceNotFound(
			final ResourceNotFoundException ex, final NativeWebRequest request) {
		
		return create(Status.NOT_FOUND, ex, request);
	}
	
	@ExceptionHandler
	default ResponseEntity<Problem> handleResourceAlreadyExists(
			final ResourceAlreadyExistException ex, final NativeWebRequest request) {
		
		return create(Status.CONFLICT, ex, request);
	}
	
	@ExceptionHandler
	default ResponseEntity<Problem> handleResourceNotAvailable(
			final ResourceNotAvailableException ex, final NativeWebRequest request) {
		
		return create(Status.SERVICE_UNAVAILABLE, ex, request);
	}
	
	@ExceptionHandler
	default ResponseEntity<Problem> handleApplicationException(
			final ApplicationException ex, final NativeWebRequest request) {
		
		return create(Status.BAD_REQUEST, ex, request);
	}
	
	@ExceptionHandler
	default ResponseEntity<Problem> handleApplicationErrorException(
			final ApplicationErrorException ex, final NativeWebRequest request) {
